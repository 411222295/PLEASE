...
export const ApplicationListPage: React.FC = () => {
  ...
  const handleFilter = (filters: any) => {
    ...
    if (filters.dateRange?.start) {
      filtered = filtered.filter(app => new Date(app.appliedDate) >= new Date(filters.dateRange.start));
    }
    if (filters.dateRange?.end) {
      filtered = filtered.filter(app => new Date(app.appliedDate) <= new Date(filters.dateRange.end));
    }
    if (filters.industry) {
      filtered = filtered.filter(app => app.job.industry === filters.industry);
    }
    if (filters.experience) {
      filtered = filtered.filter(app => app.job.experience === filters.experience);
    }
    if (filters.education) {
      filtered = filtered.filter(app => app.job.education === filters.education);
    }
        ...
        <div className="flex justify-between items-center mb-8">
          ...
          <div className="flex space-x-4">
            ...
            <button
              ...
              className="bg-white text-gray-700 px-4 py-2 rounded-md border border-gray-300 hover:bg-gray-50"
            >
              {showFilterPanel ? '隐藏高级筛选' : '显示高级筛选'}
            </button>
            <button
              onClick={() => setShowStatistics(!showStatistics)}
              className="bg-white text-gray-700 px-4 py-2 rounded-md border border-gray-300 hover:bg-gray-50"
            >
              {showStatistics ? '隐藏统计信息' : '显示统计信息'}
            </button>
            <button
